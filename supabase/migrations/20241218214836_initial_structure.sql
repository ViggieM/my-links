...
CREATE OR REPLACE FUNCTION public.get_parent_tags_and_siblings(tag_ids bigint[])
 RETURNS TABLE(id bigint, name text, parent_id smallint)
 LANGUAGE sql
AS $function$
WITH RECURSIVE
  parent_tags AS (
...
CREATE OR REPLACE FUNCTION public.get_parent_tags_multiple(tag_ids bigint[])
 RETURNS TABLE(id bigint, name text, parent_id smallint)
 LANGUAGE sql
AS $function$
